<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="入库价格" prop="rukujiage">
          <el-input v-model="ruleForm.rukujiage" 
              placeholder="入库价格" clearable  :readonly="ro.rukujiage"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="入库价格" prop="rukujiage">
              <el-input v-model="ruleForm.rukujiage" 
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="入库内容" prop="rukuneirong">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="入库内容"
                  v-model="ruleForm.rukuneirong" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.rukuneirong" label="入库内容" prop="rukuneirong">
                    <span>{{ruleForm.rukuneirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	shhf : false,
      ...
      rules: {
          ...
          rukujiage: [
                { required: true, message: '入库价格不能为空', trigger: 'blur' },
  ...
  computed: {
    zongjiage:{
      get: function () {
        return 1*this.ruleForm.rukujiage*this.ruleForm.shuliang
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `huowuruku/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huowurukuCrossAddOrUpdateFlag = false;
                  this.parent.search();
