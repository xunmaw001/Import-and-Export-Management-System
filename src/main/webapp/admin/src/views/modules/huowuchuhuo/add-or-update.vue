<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="出售客户" prop="chushoukehu">
          <el-input v-model="ruleForm.chushoukehu" 
              placeholder="出售客户" clearable  :readonly="ro.chushoukehu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="出售客户" prop="chushoukehu">
              <el-input v-model="ruleForm.chushoukehu" 
                placeholder="出售客户" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="出库价格" prop="chukujiage">
          <el-input v-model="ruleForm.chukujiage" 
              placeholder="出库价格" clearable  :readonly="ro.chukujiage"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="出库价格" prop="chukujiage">
              <el-input v-model="ruleForm.chukujiage" 
                placeholder="出库价格" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="出库内容" prop="chukuneirong">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="出库内容"
                  v-model="ruleForm.chukuneirong" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.chukuneirong" label="出库内容" prop="chukuneirong">
                    <span>{{ruleForm.chukuneirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	shhf : false,
      ...
      rules: {
          ...
          chushoukehu: [
                { required: true, message: '出售客户不能为空', trigger: 'blur' },
          ],
          chukujiage: [
                { required: true, message: '出库价格不能为空', trigger: 'blur' },
  ...
  computed: {
    zongjiage:{
      get: function () {
        return 1*this.ruleForm.chukujiage*this.ruleForm.shuliang
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.huowuleixing = true;
          ...
          if(o=='chushoukehu'){
            this.ruleForm.chushoukehu = obj[o];
	    this.ro.chushoukehu = true;
            continue;
          }
          if(o=='chukujiage'){
	    ...
	    this.ro.chukushijian = true;
          ...
          if(o=='chukuneirong'){
            this.ruleForm.chukuneirong = obj[o];
	    this.ro.chukuneirong = true;
            continue;
          }
          if(o=='yonghuming'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `huowuchuhuo/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huowuchuhuoCrossAddOrUpdateFlag = false;
                  this.parent.search();
